...
const PodcastNode: React.FC<NodeProps<ContentNodeData>> = ({ id, data }) => {
  ...
  return (
    ...
    >
      <div 
        className={`relative node-wrapper node-type-podcast`}
        style={{ '--node-color': nodeColor } as React.CSSProperties}
        onMouseDown={(e) => e.stopPropagation()}
      >
        <img src={podcastSvg} alt="Podcast" className="w-32 h-32" />
        ...
        >
          <img src={leftPodcastSvg} alt="" className="h-5 w-5" />
        </div>
        ...
        <div
          ...
          title="Add next step"
        >
          <img src={rightPodcastSvg} alt="" className="h-5 w-5" />
        </div>
