...
const NoteNode: React.FC<NodeProps<NoteNodeData>> = ({ id, data }) => {
  ...
  const editor = useEditor({
    ...
    onUpdate: ({ editor }) => {
      setNodes(nds => nds.map(node => node.id === id ? { ...node, data: { ...node.data, content: editor.getHTML() } } : node));
    },
    editorProps: {
      attributes: {
            ...
            <BubbleMenu editor={editor} tippyOptions={{ duration: 150 }}>
              ...
              >
                ...
                <button
                  ...
                  type="button"
                />
                {/* Highlight Button (was Clear Formatting) */}
                <button
                  onClick={() => editor?.chain().focus().toggleHighlight().run()}
                  disabled={!editor?.can().chain().focus().toggleHighlight().run()}
                  aria-label="Highlight"
                  style={{
                    position: 'absolute', left: '198px', top: '12px', width: '28px', height: '28px',
                    
                    cursor: 'pointer', padding: 0, pointerEvents: 'auto', borderRadius: '4px',
                  }}
                  type="button"
                />
                {/* Row 2 Buttons */}
                {/* Task List Button (was Bullet List) */}
                <button
                  onClick={() => editor?.chain().focus().toggleTaskList().run()}
                  disabled={!editor?.can().chain().focus().toggleTaskList().run()}
                  aria-label="Task List"
                  style={{
                    position: 'absolute', left: '20px', top: '45px', width: '38px', height: '28px',
                    
                    cursor: 'pointer', padding: 0, pointerEvents: 'auto', borderRadius: '4px',
                  }}
                  type="button"
                />
                {/* Ordered List Button (Remains the same) */}
                <button
                  onClick={() => editor?.chain().focus().toggleOrderedList().run()}
                  disabled={!editor?.can().chain().focus().toggleOrderedList().run()}
                  aria-label="Ordered List"
                  style={{
                    position: 'absolute', left: '64px', top: '45px', width: '28px', height: '28px',
                    
                    cursor: 'pointer', padding: 0, pointerEvents: 'auto', borderRadius: '4px',
                  }}
                  type="button"
                />
                {/* Bullet List Button (New) */}
                <button
...
// Utility: Get caret coordinates in textarea using mirror div
function getCaretCoordinates(textarea: HTMLTextAreaElement, pos: number): { top: number; left: number } | null {
  ...
  for (const prop of [
    ...
    'lineHeight', 'fontFamily', 'textAlign', 'textTransform', 'textIndent', 'textDecoration', 'letterSpacing',
  ...
  if (textarea.value.length === 0 || pos === 0) {
    div.textContent = '.';
  }
